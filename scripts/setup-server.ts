...
function stateToEnv(state: Record<string, unknown>): string {
  const s = (k: string): string => String(state[k] ?? '');
  const lines: string[] = [
...
<body>
  <div class="container">
    ...
    <div class="step" id="step-2">
      ...
      <div id="telegram-fields">
        ...
        <div class="field">
          <label>Allowed User IDs <span class="label-hint">(comma-separated)</span></label>
          <input type="text" id="allowedUserIds" placeholder="123456789,987654321">
          <div class="validation" id="userid-hint" style="color:var(--text-dim)">Get your ID from <a href="https://t.me/userinfobot" target="_blank" style="color:var(--accent)">@userinfobot</a></div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    async function writeConfig() {
      ...
      } catch (e) { btn.disabled = false; btn.textContent = 'Write .env & Finish'; alert('Failed: ' + e.message); }
    }
