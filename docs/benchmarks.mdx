...
The results on this page are reference measurements produced by the search-quality harness used during development.

The corpus is generated deterministically — the same seed always produces the same notes and queries. Each pipeline configuration is evaluated independently on the same corpus and query set so that the metric deltas are directly comparable.

<Note>
The benchmark runner (`benchmarks/search/run.ts` and `pnpm bench:search`) is not currently included in this repository. Treat this page as methodology and results documentation rather than a step-by-step runnable guide. The harness will be published as a separate tool once the benchmark suite is stabilised.
</Note>
