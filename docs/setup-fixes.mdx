...
When `ENABLE_MCP=true`, EchOS starts an HTTP server on `MCP_PORT` (default `3939`) that exposes seven knowledge tools via the Model Context Protocol. The server only binds to `127.0.0.1`.
