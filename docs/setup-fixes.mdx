...
## March 2026 — SSHFS write access via Docker UID alignment

### Problem

SSHFS mounts of the knowledge directory were read-only. Docker writes files as the `node` user
(UID 1000 inside the container), which may differ from the server SSH user's UID, making SSHFS
mounts effectively read-only even though the SSH key has full access.

### Fix

`docker-compose.yml` now sets `user: "${UID:-1000}:${GID:-1000}"` on the `echos` service,
making Docker write volume files as the host user. To activate:

```bash
# On the server — add your UID/GID to docker/.env (compose reads this for variable substitution)
echo "UID=$(id -u)" >> ~/echos/docker/.env
echo "GID=$(id -g)" >> ~/echos/docker/.env

# Re-own any files created before this fix
sudo chown -R $(id -u):$(id -g) ~/echos/data

# Restart Docker with the new user setting
cd ~/echos/docker && docker compose up -d --force-recreate
```

See [Remote Access → SSHFS write access](/remote-access#sshfs-write-access) for full details.
