...
### Enabling write access {#sshfs-write-access}
...
*Step 1 — add your UID/GID to `docker/.env` on the server (this is the file Docker Compose reads for variable substitution, not the app `.env`):*
...
*Step 2 — re-own all existing data files and restart Docker:*

```bash
# Run on the server — chown the entire data directory, not just knowledge
