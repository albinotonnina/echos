...
```bash
# Generate corpus fixtures (one-time, committed via gitignore exclusion)
...
The benchmark uses **deterministic pseudo-embeddings** derived from topic-cluster assignments — no OpenAI API key required. Results are fully reproducible: the same corpus always produces the same scores. The `hybrid+decay+hotness+rerank` configuration requires `ANTHROPIC_API_KEY` and is skipped otherwise.
