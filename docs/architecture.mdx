...
**Location:** `packages/core/src/templates/index.ts`

The template module allows users to create notes from pre-defined structures. Templates are stored as markdown files with YAML frontmatter in `<knowledgeDir>/templates/`.
...
**Tool:** `use_template` (`packages/core/src/agent/tools/use-template.ts`)

Actions: `list` (show templates), `use` (create a note from template), `create` (save a custom template). Using a template creates a real note persisted to markdown, SQLite, and the vector store.
...
**Location:** `packages/core/src/graph/auto-linker.ts`

The auto-linker module suggests links between semantically similar notes to help the knowledge graph grow organically.
...
Each result is a `LinkSuggestion`: `{ targetId, targetTitle, similarity, reason }` where `reason` is derived from shared tags, shared category, or falls back to "semantically similar content".

### `suggest_links` Agent Tool

**Location:** `packages/core/src/agent/tools/suggest-links.ts`

Standalone tool for on-demand link suggestions. Parameters: `noteId` (required), `limit` (default 5, max 20). Returns suggestions with similarity percentages and reasons. Use `link_notes` to accept any suggestions.
...
Hybrid search combines multiple strategies in a pipeline (`packages/core/src/storage/search.ts`):
...
5. **Cross-encoder reranking** (optional, default off): after all scoring stages, the top-N candidates are sent to Claude Haiku with a relevance-scoring prompt. Scores are parsed and results re-sorted. Adds one API call per search but yields the highest-quality ranking. Enable with `rerank: true` in `SearchOptions` or the `search_knowledge` tool. Requires `ANTHROPIC_API_KEY`. Implemented in `packages/core/src/storage/reranker.ts`.
...
The STT layer provides a unified transcription interface backed by multiple providers. It lives in `packages/core/src/stt/`.
...
| Local        | N/A        | `base.en` (whisper.cpp)         |

### Auto-Detection Flow

When `STT_PROVIDER=auto` (default), the system detects the provider in this order:

1. **Base URL** — if `STT_BASE_URL` matches a known provider endpoint
2. **Model name** — if `STT_MODEL` contains provider-specific hints (e.g., `SenseVoiceSmall` → SiliconFlow)
3. **API key prefix** — longer prefixes checked first (`sk-or-` before `sk-`)
4. **Cache lookup** — checks `~/.echos/stt-provider-cache.json` (24h TTL, keyed by SHA-256 hash of API key)
5. **Probe fallback** — for providers with no detectable key prefix (Together AI, SiliconFlow, DeepInfra, Fireworks AI), hits the `/models` endpoint sequentially with a 5s timeout. First 200 response wins and is cached automatically.

### Configuration

```env
# Auto-detect (default) — just set the key
...
- `registry.ts` — Provider registry with detection functions, caching, and probing
- `factory.ts` — Async `createSttClient()` with auto-detection and probe fallback
- `openai-compatible-client.ts` — OpenAI-compatible API client for cloud providers
- `local-whisper-client.ts` — Local whisper.cpp client (supports C++ binary and Python wrapper)
- `scripts/whisper-cpp-wrapper.py` — Python wrapper for the `whispercpp` package

The `sttClient` is passed to `PluginContext` and `AgentDeps`, enabling voice transcription in Telegram, YouTube fallback, and the audio plugin.
...
| File                    | Responsibility                                                                                                                                                     |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `index.ts`              | `createEchosAgent()` — wires model, API key, memories, and delegates tool creation                                                                                 |
| `create-agent-tools.ts` | `createAgentTools()` — all core tool imports and instantiation (~38 tools). Adding a new core tool only requires editing this file and the `tools/index.ts` barrel |
| `types.ts`              | `AgentDeps` and `AgentToolDeps` interfaces — stable, widely-imported types                                                                                         |
