...
|--------|------|-------------|
| `id` | UUID | Unique revision identifier |
...
Each save checks the total revision count for the note. If it exceeds 50, the oldest revisions are deleted in the same transaction. The cap is enforced in `RevisionStorage.saveRevision()`.
...
`restore_note` reverses the process: the `.md` file is moved back from `.trash/` to its original path, and `status` and `file_path` are restored in SQLite.
