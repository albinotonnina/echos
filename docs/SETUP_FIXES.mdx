...
Controls the TTL of Anthropic prompt cache markers applied to the system prompt and conversation history. Defaults to `long` (1-hour TTL).

```bash
CACHE_RETENTION=long    # default — 1h TTL, best cost savings for Anthropic
CACHE_RETENTION=short   # 5min TTL — useful during development
CACHE_RETENTION=none    # disable prompt caching entirely
```

**Important**: This setting only affects Anthropic models. When `LLM_BASE_URL` is set (custom OpenAI-compatible endpoint), `cacheRetention` is forced to `'none'` regardless of this env var, since those endpoints do not support Anthropic-style prompt caching.

The effective value is logged at startup:
```json
