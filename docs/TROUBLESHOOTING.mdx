...
**Problem**: The YouTube plugin uses `youtube-transcript-api-js` to extract transcripts. Some videos may not have captions available, or YouTube may block requests from cloud/VPS IPs.

**Symptoms**:

- Bot responds with something like "I was unable to save this video"
- Logs show: `YouTube transcript unavailable` or `IpBlocked`

**Fix**:

1. **Check if the video has captions**: Not all YouTube videos have transcripts available. Videos with auto-generated or manual captions will work.

2. **Whisper fallback**: If you have an OpenAI API key configured (`OPENAI_API_KEY`), EchOS will automatically download the audio and transcribe it using Whisper when YouTube captions are unavailable.

3. **Cloud/VPS deployments**: YouTube may block requests from cloud IPs. Configure Webshare proxy in `.env`:
   ...
   When set, the transcript fetcher routes through Webshare rotating residential proxies (`p.webshare.io`).
