...
EchOS configures prompt caching via `pi-ai`'s `cacheRetention` option, which controls the TTL of the `cache_control` markers applied to the system prompt and conversation history.
...
| `short` | 5 minutes | Useful for development or low-traffic sessions |
...
- **Custom OpenAI-compatible endpoints** (`LLM_BASE_URL` set): `cacheRetention` is forced to `'none'` regardless of `CACHE_RETENTION`. Custom endpoints do not support Anthropic-style prompt caching.
