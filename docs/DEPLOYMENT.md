...
The interactive wizard (`pnpm setup`) handles all configuration — API keys, interfaces, storage paths, and scheduler. It writes a `chmod 0600` `.env` file and creates the required data directories.

## Prerequisites

- Node.js 20+, pnpm 9+
- Docker and Docker Compose (for Docker deployments)
- `.env` file — generated by `pnpm setup` or manually from `.env.example`
