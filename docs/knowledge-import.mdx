...
1. In Obsidian: **Open folder as vault** → select `~/echos/knowledge/` (or wherever `KNOWLEDGE_DIR` points in your `.env`)
...
1. Copy/paste the file into any folder under `~/echos/knowledge/` (folder structure is free-form)
...
Files are copied to `~/echos/knowledge/{type}/{category}/{date}-{slug}.md` with EchOS frontmatter.
