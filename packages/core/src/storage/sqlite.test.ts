...
describe('SQLite Hotness', () => {
  it('recordAccess increments retrieval_count on first access', () => {
    storage.recordAccess('note-1');
    const rows = storage.getTopHot(10);
    expect(rows).toHaveLength(1);
    expect(rows[0]!.noteId).toBe('note-1');
    expect(rows[0]!.retrievalCount).toBe(1);
  });

  it('recordAccess increments retrieval_count on subsequent accesses', () => {
    storage.recordAccess('note-1');
    storage.recordAccess('note-1');
    storage.recordAccess('note-1');
    const rows = storage.getTopHot(10);
    expect(rows[0]!.noteId).toBe('note-1');
    expect(rows[0]!.retrievalCount).toBe(3);
  });

  it('getTopHot returns notes ordered by retrieval_count descending', () => {
    storage.recordAccess('note-a');
    storage.recordAccess('note-b');
    storage.recordAccess('note-b');
    storage.recordAccess('note-b');
    storage.recordAccess('note-c');
    storage.recordAccess('note-c');

    const rows = storage.getTopHot(10);
    expect(rows[0]!.noteId).toBe('note-b');
    expect(rows[0]!.retrievalCount).toBe(3);
    expect(rows[1]!.noteId).toBe('note-c');
    expect(rows[1]!.retrievalCount).toBe(2);
    expect(rows[2]!.noteId).toBe('note-a');
    expect(rows[2]!.retrievalCount).toBe(1);
  });

  it('getTopHot respects the limit parameter', () => {
    storage.recordAccess('note-a');
    storage.recordAccess('note-b');
    storage.recordAccess('note-c');

    const rows = storage.getTopHot(2);
    expect(rows).toHaveLength(2);
  });

  it('getHotness returns a map for known note IDs', () => {
    storage.recordAccess('note-x');
    storage.recordAccess('note-x');

    const map = storage.getHotness(['note-x', 'note-unknown']);
    expect(map.has('note-x')).toBe(true);
    expect(map.get('note-x')!.retrievalCount).toBe(2);
    expect(map.has('note-unknown')).toBe(false);
  });

  it('getHotness returns an empty map for an empty input', () => {
    const map = storage.getHotness([]);
    expect(map.size).toBe(0);
  });

  it('recordAccess updates last_accessed timestamp', () => {
    const before = new Date().toISOString();
    storage.recordAccess('note-ts');
    const after = new Date().toISOString();

    const map = storage.getHotness(['note-ts']);
    const lastAccessed = map.get('note-ts')!.lastAccessed;
